...
function makeLineChart(dataset, xName, yObjs, axisLables) {
  ...
  chartObj.margin = {
    top: 15, right: 60, bottom: 30, left: 50,
  };
  ...
  for (const yObj in yObjs) {
    yObjs[yObj].line = d3.svg.line().interpolate('cardinal').x(d => chartObj.xScale(chartObj.xFunct(d))).y(getYScaleFn(yObj));
  }
  ...
  chartObj.bind = function (selector) {
    ...
    chartObj.mainDiv.append('div').attr('class', 'inner-wrapper').append('div').attr('class', 'outer-box')
      .append('div')
  ...
  chartObj.render = function () {
    ...
    // Draw Lines
    for (var y in yObjs) {
      yObjs[y].path = chartObj.svg.append('path').datum(chartObj.data).attr('class', 'line').attr('d', yObjs[y].line)
    ...
    chartObj.svg.append('g').attr('class', 'y axis').call(chartObj.yAxis).append('text')
      .attr('class', 'label')
    ...
    for (var y in yObjs) {
      ...
      yObjs[y].tooltip.append('rect').attr('x', 8).attr('y', '-5').attr('width', 22)
        .attr('height', '0.75em');
    ...
    return chartObj;
    function mousemove() {
      const x0 = chartObj.xScale.invert(d3.mouse(this)[0]); const i = chartObj.bisectYear(dataset, x0, 1); const d0 = chartObj.data[i - 1]; const
        d1 = chartObj.data[i];
      try {
